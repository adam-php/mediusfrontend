...
export default function EscrowPage({ params }: EscrowPageProps) {
  ...
  const fetchEscrowDetails = async () => {
    try {
      ...
      const { data: profiles, error: profilesError } = await supabase
        .from("profiles")
        .select("*")
        .in("id", [escrowData.buyer_id, escrowData.seller_id])

      if (profilesError) throw profilesError

      const buyer = profiles.find((p) => p.id === escrowData.buyer_id)
      const seller = profiles.find((p) => p.id === escrowData.seller_id)

      setBuyerProfile(buyer || null)
  ...
  const handlePayPalApproval = async () => {
    ...
    try {
      ...
      const result = await response.json()
      setEscrow(result.escrow)
      setPaymentMessage("PayPal payment confirmed! Escrow is now funded.")
    } catch (error: any) {
      setError(error.message)
    } finally {
      setCheckingPayment(false)
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json()
        throw new Error(errorData.error || "Failed to confirm escrow")
      }
  ...
  const handleDecline = async () => {
    ...
    try {
      ...
      await supabase.from("escrow_messages").insert({
        escrow_id: escrow.id,
        sender_id: user.id,
        message: `Transaction declined by ${isSeller ? "seller" : "buyer"}.`,
        message_type: "system",
      })
      ...
      if (isSeller) {
        await supabase.from("escrow_messages").insert({
          escrow_id: escrow.id,
          sender_id: user.id,
          message: "Seller has declined the current terms. The buyer can now propose a new price.",
          message_type: "price_change_request",
        })
      }
  ...
  const getBothPartiesStatus = () => {
    ...
    if (buyerAction || sellerAction) {
      return {
        agreed: false,
        action: null,
        message: "Waiting for both parties to select the same action...",
      }
    }
        ...
        <div className="backdrop-blur-xl bg-white/5 border border-white/10 rounded-3xl p-8 text-center max-w-md shadow-2xl shadow-orange-500/10 relative z-10 animate-fade-in-up">
          ...
          <div className="relative z-10">
            ...
            <AnimatedBackButton
              ...
              className="bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600 text-white px-6 py-3 rounded-lg font-medium transition-all duration-200"
            />
          </div>
        </div>
      </div>
        ...
        <div className="max-w-6xl mx-auto">
          <div className="mb-8 animate-slide-in-down">
            ...
            <div className="flex flex-col lg:flex-row lg:justify-between lg:items-start space-y-4 lg:space-y-0">
              ...
              <button
                onClick={() => {}}
                className="backdrop-blur-sm bg-orange-500/20 hover:bg-orange-500/30 border border-orange-400/30 hover:border-orange-400/50 text-white px-6 py-3 rounded-2xl font-medium transition-all duration-300 flex items-center space-x-2 shadow-lg shadow-orange-500/10 hover:shadow-orange-500/20 hover:scale-105 transform ease-out"
              >
                <span>📊</span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div className="space-y-6">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex space-x-3">
                        <button
                          ...
                          className="flex-1 backdrop-blur-sm bg-gray-500/20 hover:bg-gray-500/30 border border-gray-400/30 hover:border-gray-400/50 text-white font-medium py-3 px-4 rounded-2xl transition-all duration-300 hover:scale-105 transform ease-out"
                        >
                          Cancel
                        </button>
                        <button
                          ...
                          className="flex-1 backdrop-blur-sm bg-blue-500/20 hover:bg-blue-500/30 border border-blue-400/30 hover:border-blue-400/50 text-white font-semibold py-3 px-4 rounded-2xl transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed hover:scale-105 transform ease-out shadow-lg shadow-blue-500/10 hover:shadow-blue-500/20"
                        >
                          Propose New Price
                        </button>
                      </div>
              ...
              <div className="backdrop-blur-xl bg-white/5 border border-white/10 rounded-3xl p-6 shadow-xl shadow-orange-500/10 hover:border-white/15 hover:bg-white/8 transition-all duration-500 ease-out animate-slide-in-left">
                ...
                <div className="relative z-10">
                  ...
                  <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
                    <span
                      className={`px-4 py-2 rounded-full text-sm font-medium border backdrop-blur-sm hover:scale-105 transition-transform duration-300 ${getStatusColor(
                        escrow.status,
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-6 space-y-2 sm:space-y-0">
                      <div className="flex items-center space-x-2">
                        <div
                          className={`w-3 h-3 rounded-full transition-all duration-300 ${escrow.buyer_confirmed ? "bg-green-500 shadow-lg shadow-green-500/50 animate-glow-gentle" : "bg-gray-600"}`}
                        ></div>
                        <span className="text-gray-300">Buyer Confirmed</span>
                      </div>
                      <div className="flex items-center space-x-2">
                        <div
                          className={`w-3 h-3 rounded-full transition-all duration-300 ${escrow.seller_confirmed ? "bg-green-500 shadow-lg shadow-green-500/50 animate-glow-gentle" : "bg-gray-600"}`}
                        ></div>
              ...
              <div
                className="backdrop-blur-xl bg-white/5 border border-white/10 rounded-3xl p-6 shadow-xl shadow-orange-500/10 hover:border-white/15 hover:bg-white/8 transition-all duration-500 ease-out animate-slide-in-right"
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-1">
                      <p className="text-gray-400 text-sm">
                        Buyer
                        {isBuyer && <span className="text-orange-400 text-xs font-normal ml-2">(You)</span>}
                      </p>
                      <p className="text-white text-lg font-medium">{buyerProfile?.username || "Loading..."}</p>
                    </div>
                    <div className="space-y-1">
                      <p className="text-gray-400 text-sm">
                        Seller
                        {isSeller && <span className="text-orange-400 text-xs font-normal ml-2">(You)</span>}
                      </p>
                      <p className="text-white text-lg font-medium">{sellerProfile?.username || "Loading..."}</p>
                    </div>
                    <div className="space-y-1">
              ...
              {escrow.status === "pending" && isBuyer && (
                <div
                  className="backdrop-blur-xl bg-white/5 border border-white/10 rounded-3xl p-6 shadow-xl shadow-orange-500/10 hover:border-white/15 hover:bg-white/8 transition-all duration-500 ease-out animate-slide-in-up"
                ...
                >
                  ...
                  <div className="relative z-10">
                    ...
                    {escrow.payment_method === "crypto" && escrow.deposit_address ? (
                      <div className="space-y-4">
                        ...
                        <div className="backdrop-blur-sm bg-gray-800/50 border border-gray-700/50 rounded-2xl p-4 relative group hover:bg-gray-800/70 transition-all duration-300">
                          ...
                          <button
                            ...
                            className="absolute top-3 right-3 text-gray-400 hover:text-orange-400 transition-colors duration-200 opacity-0 group-hover:opacity-100 hover:scale-110 transform ease-out"
                            title="Copy address"
                          >
                            📋
                          </button>
                        ...
                        {paymentMessage && (
                          <div
                            className={`px-4 py-3 rounded-2xl text-sm backdrop-blur-sm animate-slide-in-up ${
                    ...
                    ) : escrow.payment_method === "paypal" && escrow.paypal_order_id ? (
                      <div className="space-y-4">
                        <p className="text-gray-400">Complete your PayPal payment:</p>
                        <button
                          onClick={handlePayPalApproval}
                          disabled={checkingPayment}
                          className="backdrop-blur-sm bg-blue-500/20 hover:bg-blue-500/30 border border-blue-400/30 hover:border-blue-400/50 text-white px-6 py-3 rounded-2xl font-medium transition-all duration-300 disabled:opacity-50 hover:scale-105 transform ease-out shadow-lg shadow-blue-500/10 hover:shadow-blue-500/20"
                        >
                          {checkingPayment ? "Processing..." : "Complete PayPal Payment"}
                        </button>
                        {paymentMessage && (
                          <div className="backdrop-blur-sm bg-green-500/10 border border-green-400/20 text-green-300 px-4 py-3 rounded-2xl text-sm animate-slide-in-up">
                            <div className="flex items-center space-x-2">
                              <span>✅</span>
                              <span>{paymentMessage}</span>
                            </div>
                      ...
                      <div className="flex items-center space-x-3 text-gray-400">
                        ...
                        <span>Payment information is being generated...</span>
                      </div>
                    )}
                  </div>
                </div>
                ...
                >
                  ...
                  <div className="relative z-10">
                    ...
                    <div className="space-y-4">
                      ...
                      {bothPartiesStatus && (
                        <div
                          className={`rounded-2xl p-4 backdrop-blur-sm animate-slide-in-up ${
                        ...
                        >
                          ...
                          {!bothPartiesStatus.agreed && (
                            <div className="text-sm opacity-80">
                              <p>
                                Your choice:{" "}
                                {userAction ? (userAction === "release" ? "✅ Release" : "❌ Cancel") : "None"}
                              </p>
                              <p>
                                Other party:{" "}
                                {otherUserAction
                                  ? otherUserAction === "release"
                                    ? "✅ Release"
                                    : "❌ Cancel"
                                  : "None"}
                              </p>
                      ...
                      <div className="space-y-3">
                        <div className="flex space-x-3">
                          <button
                            ...
                            className={`flex-1 font-semibold py-4 px-6 rounded-2xl transition-all duration-300 hover:scale-105 transform ease-out ${
                              userAction === "cancel"
                        ...
                        {userAction && (
                          <button
                            ...
                            className="w-full backdrop-blur-sm bg-gray-500/10 text-gray-300 border-2 border-gray-500/30 hover:bg-gray-500/20 hover:border-gray-400/50 font-medium py-3 px-4 rounded-2xl transition-all duration-300 hover:scale-105 transform ease-out"
                          >
                            <div className="flex items-center justify-center space-x-2">
                              <span>↩️</span>
              ...
              {escrow.status === "completed" && (
                <div
                  className="backdrop-blur-xl bg-green-500/10 border border-green-400/20 rounded-3xl p-6 shadow-xl shadow-green-500/10 animate-slide-in-up"
                  style={{ animationDelay: "0.4s" }}
                >
                  <div className="absolute inset-0 bg-gradient-to-br from-green-500/5 to-green-600/5 rounded-3xl"></div>
                  <div className="relative z-10">
                    <div className="text-center">
                      <div className="text-green-400 text-6xl mb-4 animate-bounce-gentle">✅</div>
              ...
              {escrow.status === "refunded" && (
                <div
                  className="backdrop-blur-xl bg-orange-500/10 border border-orange-400/20 rounded-3xl p-6 shadow-xl shadow-orange-500/10 animate-slide-in-up"
                  style={{ animationDelay: "0.4s" }}
                >
                  <div className="absolute inset-0 bg-gradient-to-br from-orange-500/5 to-amber-500/5 rounded-3xl"></div>
                  <div className="relative z-10">
                    <div className="text-center">
                      <div className="text-orange-400 text-6xl mb-4 animate-bounce-gentle">🔄</div>
